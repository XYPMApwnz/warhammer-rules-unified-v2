...
check('every book shell file is precached',bookFiles.every(file=>sw.includes(`"./${file}"`)||file==='books/death-guard/index.html'&&sw.includes('DEATH_GUARD_FALLBACK')),bookFiles.filter(file=>!sw.includes(`"./${file}"`)&&!(file==='books/death-guard/index.html'&&sw.includes('DEATH_GUARD_FALLBACK'))).join(', '));
