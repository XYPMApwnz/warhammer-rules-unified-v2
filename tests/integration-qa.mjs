...
check('unified cache revision is current',sw.includes('`${CACHE_PREFIX}v2`'));
...
const versionedBookFile=file=>/\.(css|js)$/.test(file)?`"./${file}?v=2"`:`"./${file}"`;
...
check('book shell assets use release-versioned URLs',bookFiles.filter(file=>/\.(css|js)$/.test(file)).every(file=>book.includes(`./${file.replace('books/death-guard/','')}?v=2`)));
